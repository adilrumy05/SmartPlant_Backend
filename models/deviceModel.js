 ...
 */
exports.getAllDeviceStatuses = async () => {
  const query = `
    SELECT 
        sd.device_id,
        sd.device_name,
...
/**
 * Historical sensor readings for a single device over a range.
 */
exports.getDeviceHistory = async (deviceId, rangeKey) => {
  let interval;
  switch (rangeKey) {
    case '1H':
...
exports.addNewDevice = async (deviceData) => {
  const query = `
